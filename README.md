...
- **`reports/{since}_to_{today}.md`** — a recap of meetings that happened
  since `{since}`.
...
the manifest, and writes `reports/{since}_to_{today}.md` — past meetings in
the window with full per-meeting summaries.

```bash
ccs report                                  # default window: last 35 days
...
reports/YYYY-MM-DD_to_YYYY-MM-DD.md   # gitignored; one per run, named by recap window
...
### How the YouTube transcript flows into a report

The transcript is only ever raw material for the Claude prompt — it never
appears verbatim in the report. The cleaned transcript is spliced into the
...
the report output — only Claude's summary is.
