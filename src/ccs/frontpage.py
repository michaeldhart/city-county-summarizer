...
def build(today: date | None = None, count: int | None = None,
    ...
    INDEX_PATH.write_text(_render(selected, today, window_start, widened))
    return {
...
def _render(selected: list, today: date, window_start: date, widened: bool) -> str:
    """Unstyled markdown for now — the column layout is a later pass.

    The point of this shape is to make *selection* readable: what got picked, in
    what order, and from where. Styling it before that is judged would only
    flatter it.
    """
    folio = f"Vol. {FRONT_PAGE_VOLUME} · {today.strftime('%A, %B %-d, %Y')}"
    quiet = (
    ...
    for c in selected:
        pin = " 📌" if c.brief.pinned else ""
        lines += [
            f"### {c.brief.headline}{pin}",
            "",
            c.brief.body,
            "",
            f"*{c.body_title} · {_pretty(c.date)}* — "
            f"[read the dispatch]({{{{ '{c.url}' | relative_url }}}})",
            "",
        ]
    lines += [
